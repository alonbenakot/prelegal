/**
 * Common Paper Mutual Non-Disclosure Agreement — Standard Terms, Version 1.0.
 *
 * Transcribed verbatim from `templates/Mutual-NDA.md` at the repo root, with
 * two pieces of light markup preserved so both the on-screen preview and the
 * PDF can render them (see `segments.ts`):
 *
 *   - `**bold**`      — defined terms, exactly as the source marks them.
 *   - `{{Term}}`      — a term defined on the Cover Page. The source marks
 *                       these with `<span class="coverpage_link">`; they are
 *                       cross-references to the Cover Page, not blanks, so
 *                       they render as the defined term rather than being
 *                       substituted with a value.
 *
 * Do not reword this text. It is the operative language of the agreement, and
 * keeping it byte-comparable to the source template is what makes it possible
 * to diff against upstream Common Paper revisions.
 *
 * Free to use under CC BY 4.0. https://commonpaper.com/standards/mutual-nda/1.0/
 */

export type Clause = {
  /** Bold run-in heading, e.g. "Introduction". */
  title: string;
  body: string;
};

export const STANDARD_TERMS_URL =
